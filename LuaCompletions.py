...
if isST3():
    from .luacompletionslib import helper
else:
    from luacompletionslib import helper
    # reload(helper)


PACKAGE_NAME = "LuaCompletions"
LIB_PATH="luacompletionslib"
...
def settingsChanged():
    ...
    arr=["5.1","5.2"]
    if s not in arr:
        sublime.error_message(PACKAGE_NAME+": Settings->system invalid!")
    else:
...
class LuaCompletionsNewFileCommand(sublime_plugin.WindowCommand):
